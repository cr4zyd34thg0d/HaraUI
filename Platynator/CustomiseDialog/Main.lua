...
local function SetupBehaviour(parent)
  ...
  local friendlyModuleCheckbox = addonTable.CustomiseDialog.Components.GetCheckbox(container, addonTable.Locales.ENABLE_FRIENDLY_NAMEPLATES, 28, function(value)
    if InCombatLockdown() then
      return
    end
    addonTable.Config.Set(addonTable.Config.Options.ENABLE_FRIENDLY_NAMEPLATES, value)
  end)
  friendlyModuleCheckbox.option = addonTable.Config.Options.ENABLE_FRIENDLY_NAMEPLATES
  friendlyModuleCheckbox:SetPoint("TOP", allFrames[#allFrames], "BOTTOM", 0, 0)
  table.insert(allFrames, friendlyModuleCheckbox)
